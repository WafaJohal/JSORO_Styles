...
<<>>=
ch12<-subset(allchild_feat, child == "12")
@

<<plot_QOMexample,fig=T,echo=F,include=T,height=4,tidy=TRUE>>=
ggplot(ch12, aes(x=as.factor(ch12$time_rel), y=QoM, colour=activity,group=1))+ geom_line() +scale_colour_discrete_list()+ theme_Publication()
@
