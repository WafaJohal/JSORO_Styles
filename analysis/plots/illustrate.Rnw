...
<<>>=
ch12<-subset(allchild_feat, child == "8")
@

<<plot_QOMexample,fig=T,echo=F,include=T,height=4,tidy=TRUE>>=
ggplot(ch12, aes(x=as.factor(ch12$time), y=remove_outliers(QoM), colour=activity,group=session))+ geom_line() +scale_colour_discrete_list()+ theme_Publication()
@
